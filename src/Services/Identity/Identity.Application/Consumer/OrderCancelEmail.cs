...
public class OrderCancelEmail : IConsumer<OrderCancelEmailEvent> 
{
    private readonly UserManager<MeetSkoolIdentityUser> _userManager;
    private readonly IEmailServices _emailServices;

    public OrderCancelEmail(UserManager<MeetSkoolIdentityUser> userManager, IEmailServices emailServices)
    {
        _userManager = userManager;
        _emailServices = emailServices;
    }
    public async Task Consume(ConsumeContext<OrderCancelEmailEvent> context)
    {
        try
        {
            ...
            if (teacher != null)
            {
                ...
                if (teacherName != null)
                    if (teacherEmail != null)
                        await _emailServices.OrderCancelEmail(context.Message.TeacherId, teacherName, teacherEmail);
            }
            ...
            if (student != null)
            {
                ...
                {
                    await _emailServices.OrderCancelEmail(context.Message.StudentId, studentName, studentEmail);
                }
            }
        }
        catch (Exception e)

...
public class OrderCompleteEmail : IConsumer<OrderCompleteEmailEvent>
{
    private readonly UserManager<MeetSkoolIdentityUser> _userManager;
    private readonly IEmailServices _emailServices;

    public OrderCompleteEmail(UserManager<MeetSkoolIdentityUser> userManager, IEmailServices emailServices)
    {
        _userManager = userManager;
        _emailServices = emailServices;
    }
    public async Task Consume(ConsumeContext<OrderCompleteEmailEvent> context)
    {
        try
        {
            ...
            if (teacher != null)
            {
                ...
                if (teacherName != null)
                    if (teacherEmail != null)
                        await _emailServices.OrderCompleteEmail(context.Message.TeacherId, teacherName, teacherEmail);
            }
            ...
            if (student != null)
            {
                ...
                {
                    await _emailServices.OrderCompleteEmail(context.Message.StudentId, studentName, studentEmail);
                }
            }
        }
        catch (Exception e)
